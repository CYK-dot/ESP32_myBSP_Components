...
/**
 *@brief 转义状态机
 *@param stateEsacpe 转义状态机状态
 *@param byte 转义前的字节
 *@param isFrame 是否是帧头帧尾
 *@return 正数代表转义出的字符，负数代表尚未完成转义或出了问题
 *@addtogroup 状态机
**/
static inline int prvRxFsmEscape(uint8_t *stateEscape,uint8_t byte,bool *isFrame)
{
    // 如果启用转义
    #if RDLC_ESCAPE_ENABLE == 1
    switch(*stateEscape)
    ...
    // 如果关闭转义
    #else
    if (byte == BYTE_HEAD || byte == BYTE_TAIL) {
        *isFrame = true;
    }
    else {
        *isFrame = false;
 ...
 *@addtogroup 状态机
**/
static inline int prvRxFsmParse(RdlcStaticHandle_t *handle,uint8_t byte,bool isFrame)
{
    uint16_t crcFromBuf;
